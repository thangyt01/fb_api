...
import { getListFriend, getListBlockUser, getListUser, getListFriendRequest } from "./userService";
...
export class UserController {
    ...
    static async getListFriendRequest(req, res) {
        try {
            const params = {
                page: req.query.page ? parseInt(req.query.page) : 0,
                limit: req.query.limit ? parseInt(req.query.limit) : 20,
            ...
            const result = await getListFriendRequest(params)
            if (result.success) {
                res.json(respondArraySuccess(result.data, result.total))
            } else {
                res.json(respondWithError(result.code, result.message, result.data))
